    ...
    class LeftHandSwipeRightGesture : Gesture
    {
       /// <summary>
        ...
        public LeftHandSwipeRightGesture(String name) : base(name)
        {
            ...
            description = "For correct execution: 1. stay in front of the kinect 2. let your right arm hang loose  3. keep your left hand about 40 centimeters (in x direction) to the left away from your body  4. swipe your left hand constant to the right";
        }
        ...
        {
            //distance between left hand and hip left
            double HDTB = dict[JointType.HipLeft].Position.X-dict[JointType.HandLeft].Position.X;
            //distance between right hand and right hip
            double lhhs = dict[JointType.HandRight].Position.X-dict[JointType.HipRight].Position.X;

            if (dict[JointType.HandLeft].Position.X < 0 && HDTB >= 0.4 && lhhs <= 0.3)
            {
